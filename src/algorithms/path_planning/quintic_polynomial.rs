...
impl QuinticPolynomial {
	pub fn new(
		...
		Ok(Self {
			cx: get_coefficients(x0.0, v0.0, a0.0, x1.0, v1.0, a1.0, t1),
			cy: get_coefficients(x0.1, v0.1, a0.1, x1.1, v1.1, a1.1, t1),
			max_t: t1,
		})
	}
	...
	pub fn velocity(&self, t: f64) -> (f64, f64) {
		let t_2 = t * t;
		let t_3 = t_2 * t;
		let t_4 = t_3 * t;
	...
	pub fn jerk(&self, t: f64) -> (f64, f64) {
		let t_2 = t * t;
		(
	...
	pub fn evaluate(&self, t: f64) -> Result<(f64, f64), QuinticError> {
	...
	pub fn evaluate_unchecked(&self, t: f64) -> (f64, f64) {
		let t_2 = t * t;
		let t_3 = t_2 * t;
		let t_4 = t_3 * t;
		let t_5 = t_4 * t;
...
mod tests {
	...
	fn quintic_polynomial_evaluate() {
		...
		assert!((value.0 - 5.9326171875).abs() < 1e-10);
		assert!((value.1 - (-1.1435546875)).abs() < 1e-10);
	...
	fn solved_quintic() {
		...
		let tuple_correct = |o: (f64, f64), e: (f64, f64)| -> bool {
			(o.0 - e.0).abs() < 0.00001 && (o.1 - e.1).abs() < 0.00001
		};

		let check_correct = |expected_x, expected_v, expected_a, t| -> bool {
			tuple_correct(graph.evaluate(t).unwrap(), expected_x)
				&& tuple_correct(graph.velocity(t), expected_v)
				&& tuple_correct(graph.acceleration(t), expected_a)
		};
